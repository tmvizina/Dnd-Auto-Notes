...
| `94d006f` | `P2-03` | Cross-track transcript stage, overlap and mic-bleed merge | Independent review passed the runStage composition and deterministic merge. Focused coverage runs VAD and ASR for every manifest track, proves byte-stable skip behavior, stable six-digit ids, symmetric overlap marking, louder-copy bleed selection with `MIC_BLEED` QA, backchannels, word-gap splitting and per-player VAD totals. Core typecheck, lint and format passed. |
| `12d7b10` | `P2-04` | Batched utterance embeddings, prosody and reader-safe binary feature store | Independent review returned missing jitter/custom-I/O/batching defects, then padded-batch determinism and finally JSON/blob publication races. It passed after all nine raw and per-player-z-scored prosody fields were covered, ECAPA received relative lengths in bounded batches, and immutable content-addressed blobs were published before an atomic JSON pointer. Focused tests passed 3 TypeScript and 5 Python cases plus core typecheck. Deviation: the active blob is `features.<sha256>.bin`, not a mutable `features.bin`, so old readers remain consistent during reruns and failed publication. Real-session throughput remains measured by `P5-02`. |
| `70f52cc` | `P4-11` | Validated settings UI, persistence and private-network connection test | Independent review returned unwired runtime settings, arbitrary-network SSRF, DNS-rebinding and misleading sidecar-path defects, then passed after startup consumed path settings, supported directory shapes were enforced, and the models probe pinned a validated private/LAN IP while preserving Host/TLS identity and disabling redirects. Focused coverage passed 54 tests and the full JavaScript suite passed 443, with desktop/UI typechecks. Provider, ASR and threshold values explicitly say they are saved for later consumers rather than falsely claiming live effect; credentials are rejected and never persisted. |
| `b25f2c9` | `P2-06` | Pure weighted lexical evidence, quote spans, spoken-number normalization and campaign lexicon | Independent review returned curly-quote span, duplicate-player-name and malformed-override defects, then passed after exact Unicode quote spans, stable speaker-id suppression and structured lexicon validation were added. Focused coverage passed 22 tests with core typecheck. The engine is deterministic, reports every contributing marker, and leaves roll-total matching explicit for P2-09 alignment. |
| `343429b` | `P2-04` | Corrective lint and formatting pass for the integrated features stage | Removed four dead declarations left in the earlier P2-04 integration and applied the repository formatter to its source and tests. The focused features suite passed 3 tests; core typecheck plus repository-wide lint and format checks passed. No runtime behavior changed. |
